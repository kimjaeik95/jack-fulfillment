 ...
 *
 * 재고 한 줄은 '빈 × SKU × 거래처' 다. 같은 SKU 라도 빈이 다르면 다른 줄이고,
 * 위탁이면 거래처까지 갈라진다 — 남의 물건과 우리 물건을 한 줄에 섞으면
 * 정산이 안 된다.
 *
 * 자사 재고는 거래처가 비어 있다. 위탁 기능이 아직 없어 지금은 전부 그쪽이다.
 * '이 물건이 어느 공급처에서 왔나' 는 재고가 아니라 이력에서 본다 —
 * 재고 → 이력(ref_type='INBOUND') → 입고 → 공급처.
 *
