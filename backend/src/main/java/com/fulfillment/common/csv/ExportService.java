...
public class ExportService {
	...
	public byte[] orgs(LoginUser actor, OrgSearch search, ExportFormat format) {
		...
		TableWriter csv = format.newWriter("조직", "조직코드", "조직명", "조직유형", "회사코드",
				"회사명", "상위조직코드", "상위조직명",
				"담당자", "연락처", "주소", "우편번호",
				"소속인원", "하위조직수", "플랜트수", "정렬순서", "사용여부", "등록일시");
		for (OrgResponse r : rows) {
			csv.row(r.orgId(), r.orgName(), r.orgType(), r.companyId(), r.companyName(),
					r.parentId(), r.parentName(),
					r.managerName(), r.phone(), r.address(), r.zipCode(),
					r.userCount(), r.childCount(), r.plantCount(),
