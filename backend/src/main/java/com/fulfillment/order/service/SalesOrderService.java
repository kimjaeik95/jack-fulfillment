...
/**
 * 주문 등록 · SKU 매핑 · 확정 (ORD-PG-009 ~ 011).
 *
 ...
 *
 *   1 유형에 따라 필수가 다르다
 *     채널주문은 채널이, 판매오더는 고객이 있어야 한다. 둘 다 없으면 어느
 *     쪽인지 알 수 없고, 둘 다 있으면 어느 쪽으로 정산할지 정할 수 없다.
 *
 *   2 같은 채널의 같은 주문번호는 한 번만 (ORD-001)
 *     OMS 가 재전송해도 한 건이어야 한다. 유니크 제약이 마지막 방어선이지만
 *     그 전에 사람이 읽을 수 있는 메시지로 돌려준다.
 *
 *   3 변환 실패한 라인을 버리지 않는다 (ORD-005)
 *     외부코드가 매핑에 없으면 SKU 없이 적재하고 오류대기로 보낸다. 버리면
 *     고객은 주문했는데 우리에게는 없는 상태가 된다.
 *
 *   4 미매핑이 남아 있으면 확정하지 않는다 (ORD-PG-011)
