...
INSERT INTO tb_role_permission (role_seq, perm_seq, action_code, created_by)
SELECT r.role_seq, p.perm_seq, a.action_code, 'system'
  FROM (VALUES ('PURCHASER', 'MST_PARTNER', 'RCU')) AS v(role_id, perm_id, actions)
  JOIN tb_role r       ON r.role_id = v.role_id
  JOIN tb_permission p ON p.perm_id = v.perm_id
 CROSS JOIN LATERAL unnest(string_to_array(v.actions, NULL)) AS a(action_code);
...
-- 공급처
-- ============================================================================
INSERT INTO tb_partner (partner_id, partner_name, biz_reg_no, ceo_name, manager_name,
                        phone, email, zip_code, address, status, pay_term,
                        over_receipt_rate, remark, sort_order, supplier_yn, created_by) VALUES
  ('SUP-001', '대한섬유',   '124-81-00001', '김대한', '박과장',
   '031-1234-5678', 'park@daehan.co.kr', '10393', '경기도 고양시 일산동구 중앙로 1',
   'ACTIVE', 'NET30', 3.00, '주거래. 티셔츠 · 셔츠', 10, 'Y', 'system'),
  ('SUP-002', '한성어패럴', '214-81-00002', '이한성', '최대리',
   '06236', 'choi@hansung.co.kr', '06236', '서울특별시 강남구 테헤란로 200',
   'ACTIVE', 'MONTHLY', 5.00, '팬츠 전문', 20, 'Y', 'system'),
  ('SUP-003', '누리액세서리', '312-81-00003', '정누리', NULL,
   '051-777-8888', NULL, '48058', '부산광역시 해운대구 센텀중앙로 90',
   'ACTIVE', 'PREPAID', 0.00, '가방 · 잡화. 소량 거래', 30, 'Y', 'system'),
  -- 거래를 멈춘 곳도 하나 둔다. 목록에서 구분이 보이는지 확인하려면 필요하다.
  ('SUP-004', '구일텍스',   '408-81-00004', '한구일', '윤사원',
   '053-222-3333', NULL, '41585', '대구광역시 북구 칠성로 10',
   'SUSPENDED', 'NET60', 0.00, '품질 이슈로 거래 중지 (2026-06)', 40, 'Y', 'system');

-- 연락처 자리에 우편번호가 들어간 SUP-002 를 바로잡는다.
-- 시드 오타를 그대로 두면 검증에서 '연락처 형식' 을 확인할 수 없다.
UPDATE tb_partner SET phone = '02-555-0002' WHERE partner_id = 'SUP-002';


-- ============================================================================
-- 고객으로 쓰는 거래처
--
-- 고객유형(B2B/B2C)을 두지 않는다. B2C 는 애초에 이 테이블에 들어오지 않기
-- 때문이다 — 오픈마켓 주문은 수령인을 주문이 스냅샷으로 들고 가고, 전화로
-- 들어온 단체주문도 마찬가지다. 여기 남는 것은 전부 계속 거래하는 상대다.
--
-- CUS-003 은 개인 이름이지만 반복 거래하는 직거래처라 남긴다.
-- ============================================================================
INSERT INTO tb_partner (partner_id, partner_name, biz_reg_no,
                        manager_name, phone, email, status, pay_term,
                        remark, sort_order, customer_yn, created_by) VALUES
  ('CUS-001', '무신사스토어', '120-81-10001', '김바이어',
   '02-1234-0001', 'buyer@musinsa.example', 'ACTIVE', 'NET30',
   '홀세일 계정', 10, 'Y', 'system'),
  ('CUS-002', '이천리테일',  '130-81-10002', '박구매',
   '031-500-0002', NULL, 'ACTIVE', 'MONTHLY',
   '지역 편집샵', 20, 'Y', 'system'),
  ('CUS-003', '홍길동',      NULL, NULL,
   '010-1111-2222', 'hong@example.com', 'ACTIVE', 'PREPAID',
   '직거래 개인고객', 30, 'Y', 'system'),
  ('CUS-004', '폐업상사',    '140-81-10004', NULL,
   '02-999-0004', NULL, 'CLOSED', NULL,
   '2026-03 폐업', 40, 'Y', 'system');


-- ============================================================================
-- 주소 — 거래처당 기본 1건 (MST-010)
--
-- 전부 배송지(SHIP)다. 반품지는 칼럼만 있고 아직 쓰는 화면이 없어 넣지
-- 않는다 — 쓰이지 않는 데이터를 미리 깔면 화면에서 그것이 진짜인지 시드인지
-- 구분할 수 없다.
--
...
-- ============================================================================
INSERT INTO tb_partner_address (partner_seq, address_name, receiver_name, phone,
                                zip_code, address, address_detail, delivery_memo,
                                default_yn, sort_order, addr_type, created_by)
SELECT c.partner_seq, v.address_name, v.receiver_name, v.phone,
       v.zip_code, v.address, v.address_detail, v.delivery_memo,
       v.default_yn, v.sort_order, 'SHIP', 'system'
  FROM (VALUES
       ...
       ) AS v(partner_id, address_name, receiver_name, phone, zip_code,
              address, address_detail, delivery_memo, default_yn, sort_order)
  JOIN tb_partner c ON c.partner_id = v.partner_id;
