...
```
V1__system.sql                      ← 수정 금지
V13__add_user_last_password_try.sql ← 이렇게 추가
```

### 예외 — 운영 배포 전의 정리

아직 운영 DB가 없는 동안은 한 번 몰아서 정리할 수 있습니다. 실제로 한 번 했습니다 —
23장이던 것을 12장으로 합쳤습니다. 뒤 파일이 앞 파일을 뒤집고 있어서(공급처와 고객을
따로 만들었다가 합치고, 재고 컬럼을 더했다가 순서를 바꾸고) 지금 스키마를 알려면
23장을 순서대로 따라가야 했기 때문입니다.

정리할 때는 **정리 전 스키마를 pg_dump로 떠 두고 정리 후와 대조**하세요.
컬럼 587개 · 제약 265개가 한 글자도 다르지 않아야 합니다.

운영 배포 후에는 이 예외가 사라집니다.
